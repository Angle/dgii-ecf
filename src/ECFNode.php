...
abstract class ECFNode implements ECFNodeInterface
{
        ...
        foreach ($data as $key => $value) {
            ...
            if ($propertyType == self::PROPERTY_BASE_ATTRIBUTE) {
                ...
                if ($propertyName === null) {
                    ...
                    throw new ECFException(sprintf("Invalid Attribute Name given, '%s' not found in %s object definition.", $key, $this->getShortName()), -1); // TODO: Add a proper error code
                }

                $setter = 'set' . ucfirst($propertyName);
                if (!method_exists(static::class, $setter)) {
                    // Setter not found
                    throw new ECFException(sprintf("Property '%s' has no setter method in %s", $propertyName, $this->getShortName()), -1); // TODO: Add a proper error code
                }

                // If the setter fails, it'll throw a ECFException. We'll let it arise, the final library user should be the one catching and handling these type of exceptions.
                $this->$setter($value);

            } elseif ($propertyType == self::PROPERTY_CHILDREN) {
                // Process each child node
                if (!is_array($value)) {
                    throw new ECFException(sprintf("Data key '%s' is a Child Node of %s, expecting an array with the child properties, instead got: %s", $key, $this->getShortName(), gettype($value)), -1); // TODO: Add a proper error code
                }

                $this->setChildFromData($key, $value);

            } elseif ($propertyType == self::PROPERTY_VALUE) {
                $this->setValueFromData($value);
            } else {
                throw new ECFException(sprintf("Data key '%s' is not a valid property for %s.", $key, $this->getShortName()), -1); // TODO: Add a proper error code
            }
        }
    }
     ...
     * @throws ECFException
     */
    public function setValueFromData(mixed $value): void
    {
        $setter = 'setValue';
        if (!method_exists(static::class, $setter)) {
            // Setter not found
            throw new ECFException(sprintf("Class '%s' has no setValue method for value %s", $this->getShortName(), $value)); // TODO: Add a proper error code
        }
    ...
    public function setChildFromData(string $childName, array $data): void
    {
        ...
        if (!$properties) {
            throw new ECFException(sprintf("Missing child properties definition in %s for child '%s'", $this->getShortName(), $childName), -1); // TODO: Add a proper error code
        }

        if (!array_key_exists('class', $properties) || !array_key_exists('type', $properties)) {
            throw new ECFException(sprintf("Invalid child properties definition in %s for child '%s'", $this->getShortName(), $childName), -1); // TODO: Add a proper error code
        }
        ...
        if ($childType == ECFNode::CHILD_UNIQUE) {
            // this is the only child of it's type, we can attach it with it's setter
            $setter = 'set' . ucfirst($propertyName);
            if (!method_exists(static::class, $setter)) {
                // Setter not found
                throw new ECFException(sprintf("Setter method '%s' does not exist in %s", $setter, $this->getShortName()), -1); // TODO: Add a proper error code
            }
        ...
        } elseif ($childType == ECFNode::CHILD_ARRAY) {
            ...
            if (!method_exists(static::class, $append)) {
                // Setter not found
                throw new ECFException(sprintf("Append method '%s' does not exist in %s", $append, $this->getShortName()), -1); // TODO: Add a proper error code
            }

            // now we must initialize and append a child for every item in the array
            foreach ($data as $k => $v) {
                if (!is_array($v)) {
                    throw new ECFException(sprintf("Child '%s' is registered as array in %s, expecting an array of arrays, got '%s' in child %d", $childName, $this->getShortName(), gettype($v), $k), -1); // TODO: Add a proper error code
                }
            ...
            throw new ECFException(sprintf("Invalid child type definition in %s for child '%s'", $this->getShortName(), $childName), -1); // TODO: Add a proper error code
        }
    }
    ...
    public static function createFromDOMNode(DOMNode $node): self
    {
        ...
        if ($node->hasAttributes()) {
            foreach ($node->attributes as $attr) {
                ...
                if (strpos($attr->nodeValue, '|') !== false) {
                    throw new ECFException(sprintf("Pipe character '|' cannot appear on any attribute value. [%s - %s: %s]", $node->nodeName, $attr->nodeName, $attr->nodeValue));
                }
                */
        ...
        try {
            $ecfNode = new static($attributes);
            $ecfNode->setChildrenFromDOMNodes($children);
    ...
    public function getAttributes(): array
    {
        ...
        if (!$this->validate()) {
            throw new ECFException($this->getShortName() . ' is not validated, cannot pull attributes');
        }
        ...
        foreach (static::$attributes as $key => $prop) {
            ...
            if ($value instanceof DateTime) {
                $value = $value->format(ECFNode::DATETIME_FORMAT);
            }

            if ($prop['type'] == ECFNode::ATTR_REQUIRED && ($value === null || $value === "")) {
                // Required property is not set
                throw new ECFException(sprintf("Property '%s' is required in %s", ucfirst($key), $this->getShortName()));
            }
